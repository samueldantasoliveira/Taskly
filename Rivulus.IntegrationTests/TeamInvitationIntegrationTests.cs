...
public class TeamInvitationIntegrationTests(RivulusApiFactory factory) : IClassFixture<RivulusApiFactory>
{
    [Fact]
    ...
    public async Task Owner_CanListAndRevokePendingInvitation()
    {
        ...
        client.DefaultRequestHeaders.Authorization = new AuthenticationHeaderValue("Bearer", owner.Token);
        var team = await new TeamTestHelper(client).CreateTeamAsync();
        var created = await (await client.PostAsJsonAsync($"/api/team/{team.Id}/invitations", new { Email = "future@rivulus.test" })).Content.ReadFromJsonAsync<TeamInvitationResponseDto>();

...
describe('ThemeToggle', () => {
  it('inicia no modo noturno quando não há preferência salva', () => {
    ...
    expect(document.querySelector('meta[name="theme-color"]')).toHaveAttribute('content', '#101813')
  })
  ...
  it('alterna pelo teclado e mantém a escolha após uma nova inicialização', async () => {
    ...
    expect(document.querySelector('meta[name="theme-color"]')).toHaveAttribute('content', '#f5f7f5')
    unmount()
    delete document.documentElement.dataset.theme
    loadTheme()
    render(<ThemeToggle />)
  ...
  it('usa o padrão noturno quando a preferência salva é inválida', () => {
    localStorage.setItem('taskly.theme', 'invalid')
    loadTheme()
    expect(document.documentElement).toHaveAttribute('data-theme', 'dark')
  })
  ...
  it('sincroniza a preferência entre abas e volta ao padrão se ela for removida', () => {
    loadTheme()
    render(<ThemeToggle />)
    act(() => window.dispatchEvent(new StorageEvent('storage', { key: 'taskly.theme', newValue: 'light' })))
    expect(screen.getByRole('button', { name: 'Ativar modo noturno' })).toBeInTheDocument()
    act(() => window.dispatchEvent(new StorageEvent('storage', { key: 'taskly.theme', newValue: null })))
    expect(screen.getByRole('button', { name: 'Ativar modo claro' })).toBeInTheDocument()
  })
})

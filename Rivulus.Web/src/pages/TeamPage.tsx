...
export function TeamPage() {
  ...
  const editTeamMutation = useMutation({ mutationFn: (data: z.infer<typeof teamSchema>) => updateTeam(teamId, { ...data, version: teamQuery.data?.version }), onSuccess: () => { refreshTeam(); showToast('Equipe atualizada.'); setModal(null) }, onError: (error) => showToast(error instanceof ApiError ? error.message : 'Não foi possível atualizar a equipe.', 'error') })
  ...
  return (
    <div className="page-stack">
      ...
      <div className="content-grid content-grid--team">
        ...
        <aside className="content-aside">
          ...
          <div className="member-list">
            {membersQuery.data?.slice(0, 8).map((member) => <div className="member-row" key={member.id}><Avatar name={member.name} avatarKey={member.avatarKey} size="sm" /><span><strong>{member.name}</strong><small>{member.email}</small></span>{member.isOwner && <Crown size={15} className="owner-icon" />}</div>)}
          </div>
      ...
      <Modal open={modal === 'members'} title="Gerenciar membros" description="Busque uma pessoa pelo e-mail cadastrado." onClose={() => { setModal(null); setFoundUser(null) }} size="lg">
        ...
        {foundUser && <div className="search-result"><Avatar name={foundUser.name} avatarKey={foundUser.avatarKey} /><span className="search-result__details"><strong>{foundUser.name}</strong><small>{foundUser.email}</small></span><Button size="sm" loading={addMutation.isPending} onClick={() => addMutation.mutate(foundUser.id)}>Adicionar</Button></div>}
        <div className="member-manager"><h3>Membros atuais</h3>{membersQuery.data?.map((member) => <div className="member-row member-row--managed" key={member.id}><Avatar name={member.name} avatarKey={member.avatarKey} size="sm" /><span><strong>{member.name}</strong><small>{member.email}</small></span>{member.isOwner ? <span className="badge badge--warning">Responsável</span> : <Button variant="ghost" size="sm" loading={removeMutation.isPending && removeMutation.variables === member.id} onClick={() => removeMutation.mutate(member.id)}>Remover</Button>}</div>)}</div>
      </Modal>

  ...
  return (
    <div className="page-stack">
      ...
      <Modal open={modal === 'members'} title="Gerenciar membros" description="Busque uma pessoa pelo e-mail cadastrado." onClose={() => { setModal(null); setFoundUser(null) }} size="lg">
        ...
        {foundUser && <div className="search-result"><Avatar name={foundUser.name} avatarKey={foundUser.avatarKey} /><span><strong>{foundUser.name}</strong><small>{foundUser.email}</small></span><Button size="sm" loading={addMutation.isPending} onClick={() => addMutation.mutate(foundUser.id)}>Adicionar</Button></div>}
